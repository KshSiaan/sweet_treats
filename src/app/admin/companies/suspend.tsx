...
import { BusinessUserType, UserType } from "@/types/auth";
import { useMutation, useQueryClient } from "@tanstack/react-query";
import { adminSuspendToggle } from "@/lib/api/admin";
import { toast } from "sonner";
import { useCookies } from "react-cookie";
import { cn } from "@/lib/utils";
export default function Suspend({ data }: { data: BusinessUserType }) {
  ...
  const { mutate, isPending } = useMutation({
    ...
    onSuccess: (res) => {
      ...
      qcl.invalidateQueries({ queryKey: ["admin-business-management"] });
    },
  });
